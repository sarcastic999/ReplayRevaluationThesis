...
function [ results ] = mvpa_Loc1XALLPHASE2_Face_Scene( subject, class_args )
    ...
    if exist(regsFile_train) % only for the subjects who have the regs
        ...
        for maskIdx = whichMasks % 1:length(maskfn);
         ...
         outfile = sprintf('%s/Loc1XALLPHASE2_subject%d_%s_%s_%s.mat', outfold, subject, class_args.train_funct_name, class_args.test_funct_name, processed_params) ;
