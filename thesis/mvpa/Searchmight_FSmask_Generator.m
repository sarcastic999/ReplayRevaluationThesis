function [am pm] = Searchmight_FSmask_Generator()
...
%    regsloc2FS.conds(find(regsloc2FS.conds~=0)) = regsloc2FS.conds(find(regsloc2FS.conds~=0)) + 2;
...
%     % Segregate the Faces
%     labels(find(labelsGroup==1)) = labels(find(labelsGroup==1)) *2;
%     
%    examples = TRdata; labels = TRlabelsGroup;labelsGroup(1:2:numel(labels))=1;labelsGroup(2:2:numel(labels))=2;
    ...
    % Evenly Separate the Labels
    labelsGroup(find(labels==1)) = 1:count(labels==1);
    labelsGroup(find(labels==2)) = 1:count(labels==2);
    ...
    hc=suptitle(['accuracy map for subject ' subject]); set(hc,'FontSize',8);
    
    FSmask = zeros(dimx,dimy,dimz);
    FSmask(meta.indicesIn3D(find(pm<=nThresh))) = 1;
    
    figure;
    for iz = 1:dimz
      subplot(nrows,ncols,iz);
      imagesc(FSmask(:,:,iz)',[0 1]); axis square;
      set(gca,'XTick',[]); set(gca,'YTick',[]);
      %if iz == dimz; hc=colorbar('vert'); set(hc,'FontSize',8); end
    end
    hc=suptitle(['accuracy map for subject ' subject]); set(hc,'FontSize',8);
