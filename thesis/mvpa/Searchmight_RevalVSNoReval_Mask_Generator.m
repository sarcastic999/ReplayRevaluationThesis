...
for subject = VALID_SUBJECTS
    outfile = sprintf('%s/RevalVSNoReval_%smask_subject%d.mat', outfold, brain_mask_size, subject);
    if exist(outfile)
        continue;
    end
    ...
    % Revals are indices 2 and 4.
    imregs{find(mdp_conditions_order==2)}.conds = imregs{find(mdp_conditions_order==2)}.conds * 2;
