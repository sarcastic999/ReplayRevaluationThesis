...
function [ results ] = mvpa_Loc1XRest_Motor( subject, class_args )
    baseDirFSL = '/jukebox/norman/reprev/';
    
    maskExt         = { '.nii'; '.nii'; '.img'; '.nii'; '.nii';  };
    maskfn          = { 'LR_mask' ;'FS_mask' ; 's8FSmask'; 'joint_func_roi'; 'MASK_FACE_SCENE'};
    ...
    outfold = sprintf('%s/MVPA_Rest_Motor',datapath);
    if ~exist(outfold)
        mkdir(outfold);
    end;
    results =[];
    ...
    reg_loc1_file   = 'NEWregLoc_MOTOR_RepReval_2CAT_SPM_shift3_dur3_1run';
    ...
    if exist(regsFile_train) % only for the subjects who have the regs
        ...
        raw_loc_filenames   = [run_nifti_dir 'filtered_func_data.nii'];
        load(regsFile_train);
        regsloc = regs;
        %% pad volumes
