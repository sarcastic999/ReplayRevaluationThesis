...
function [ results ] = mvpa_Loc2XRest_Face_Scene( subject, class_args )
    ...
    localizer_regs = loc2_createRegressor(subject);
    runtime     = datestr(now);
    %% MASK
    ...
    raw_filenames = {raw_loc_filename ;    raw_ima_filenames{1} ;...
        raw_ima_filenames{2};...
        raw_ima_filenames{3};...
        raw_ima_filenames{4}};
    ...
    for maskIdx = whichMasks % 1:length(maskfn);
     ...
     outfile = sprintf('%s/Loc2Shift%d_%dTRsXRest_subject%d_%s_%s_%s.mat', outfold, loc2_shiftTRs, loc2_numTRs, subject, class_args.train_funct_name, class_args.test_funct_name, processed_params) ;
     %outfile = sprintf('%s/Loc2XRest_subject%d_%s_%s_%s.mat', outfold, subject, class_args.train_funct_name, class_args.test_funct_name, processed_params) ;
