function [ classifier_performance ] = loc1Xclassifier_crossvalidator()
    %% Load Classifiers
    load_classifiers;
    ...
    subject = str2num(taskId);
    
    if (subject == 10 || subject == 24)
        return;
    end
    environment_setup;
    ...
    raw_volume_filename = sprintf('/jukebox/norman/reprev/subj/newS%d/NII/prestat_loc/%s/filtered_func_data.nii',subject,feat_dir);
    ...
    regs_folder     = ['/jukebox/norman/reprev/behavioral/newS' num2str(subject) '/onsets/'];
    reg_loc1_file   = 'NEWregLoc_RepReval_3CAT_SPM_shift3_dur3_1run';
    regsFile_train  = [regs_folder reg_loc1_file '.mat'];
    load(regsFile_train);
    regsloc = regs;
    if localizerLength<length(regs.conds) % shorten regs
        regsloc.conds = regs.conds(:,1:localizerLength);
        regsloc.runs  = regs.runs(1: localizerLength);
    else
        % padd
        regsloc = regs;
        regsloc.conds(:, end+1:localizerLength) = 0;
        regsloc.runs(end+1:localizerLength)     = max(regs.runs);
    end
...
save(sprintf('MVPA_xval/loc1/subject%d_classifier_xval.mat', subject),'classifier_performance', 'classifier_results');
end