function [am pm maps] = Searchmight_RevalVSNoReval_Mask_Generator()
...
for subject = VALID_SUBJECTS
    outfile = sprintf('%s/RevalVSNoReval_nonoise_%smask_subject%d.mat', outfold, brain_mask_size, subject);
    if exist(outfile)
        continue;
    end
    ...
    %% Condition Order
    load(sprintf('/jukebox/norman/reprev/behavioral/newS%d/run1.mat',subject),'exp');
    mdp_conditions_order = exp.MDP_rev_i;
    nonoise_runs = find(mdp_conditions_order <= 2); % 1 = no noise no reval, 2 = no noise reval.
    for run = nonoise_runs
    ...
    raw_filenames = {raw_ima_filenames{nonoise_runs(1)} ;...
    raw_ima_filenames{nonoise_runs(2)}};

    %% combine the regressors
    clear regs;
    
    regs.conds = [imregs{nonoise_runs(1)}.conds  imregs{nonoise_runs(2)}.conds];
    regs.runs  = [imregs{nonoise_runs(1)}.runs imregs{nonoise_runs(2)}.runs];
    ...
    %% Reorder Regressors and Selectors for Reval VS No Reval
    % run 2 = reval condition.
    TRlabels = sum([imregs{nonoise_runs(1)}.conds*(1+(mdp_conditions_order(nonoise_runs(1))==2))  imregs{nonoise_runs(2)}.conds*(1+(mdp_conditions_order(nonoise_runs(2))==2))]);
    TRlabelsGroup = regs.runs;
    ind = find(TRlabels==1);
    TRlabelsGroup(ind(2:2:end)) = 2;
    ind = find(TRlabels == 2);
    TRlabelsGroup(ind(1:2:end)) = 2;
