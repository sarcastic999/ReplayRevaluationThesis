...
for i = 1:numel(LOCALIZER_OPTIONS)
    if i == 1 % no TR manipulations for loc 1.
        shift_thresh = 2;
        select_thresh = 2;
    else
        shift_thresh = 1;
    ...
    for ii = shift_thresh:numel(TRSHIFT_OPTIONS)
        for iii = select_thresh:numel(TRSELECT_OPTIONS)
            for iv = 1:numel(DATARANGE_OPTIONS)
                if iv == 5 || iv == 6
                    prune_limit = 1;  % pruning is only for rest
                else
                    prune_limit = numel(DATA_PRUNING_OPTIONS);
                end
                for v = 1:prune_limit
                    setenv('SGE_LOCALIZER', LOCALIZER_OPTIONS{i}); % required, choices: 1, 2, 12, 21
                    setenv('SGE_ROI', 'FFA_PPA'); % required, choices: FFA_PPA
                    setenv('LOC2TRSHIFT', TRSHIFT_OPTIONS{ii}) % optional, default:3, choices: 2, 3
                    setenv('LOC2TR', TRSELECT_OPTIONS{iii}); % optional, default:'3', choices: 2, 3
                    setenv('SGE_DATARANGE', DATARANGE_OPTIONS{iv}); % required, choices: REST, MINIREST1, MINIREST2, MINIREST3,  ALLPHASE2, PHASE2NOREST,
                    setenv('SGE_DATA_PRUNING', DATA_PRUNING_OPTIONS{v}); % optional, default: '', choices: 'REST_TRUNCATE_1', 'REST_TRIM_5', 'REST_TRIM_AND_TRUNCATE'
