...
function [ results ] = mvpa_Loc12XPHASE2NOREST_Face_Scene( subject, class_args )
    ...
    outfold = sprintf('%s/MVPA_PHASE2NOREST_Face_Scene',datapath);
    if ~exist(outfold)
        mkdir(outfold);
    end;
    results =[];
    ...
    if epiLenTrain<length(regs.conds) % shorten regs
        localizer1_regs.conds = regs.conds(:,1:epiLenTrain);
        localizer1_regs.runs  = regs.runs(1: epiLenTrain);
    else
        % pad
        localizer1_regs = regs;
    ...
    %% load localizer 2 regs - first 4 subjects do not have localizer 2
    if subject < 5
    ...
    for run = 1:4
        ...
        clear regs;
        %% b) test set
        allphase2regs = mvpa_phase2_selector_generator(subject, run);
        load(regsFile_test);
        restregs = regs;
        allphase2regs.conds(find(restregs.conds==1)) = 0; % deselect the rest periods
        regs = allphase2regs;
    ...
    raw_filenames = {raw_loc1_filename ; raw_loc2_filename ;    raw_ima_filenames{1} ;...
        raw_ima_filenames{2};...
        raw_ima_filenames{3};...
        raw_ima_filenames{4}};
    %% 6c) combine the regressors
    clear regs;
    regslocFS.conds = localizer1_regs.conds(1:2, :);
    regslocFS.runs = localizer1_regs.runs;
    regslocFS.conds = [regslocFS.conds localizer2_regs.conds(1:2, :)];
    regslocFS.runs = [regslocFS.runs localizer2_regs.runs];
    ...
    for maskIdx = whichMasks % 1:length(maskfn);
     ...
     outfile = sprintf('%s/Loc12Shift%d_%dTRsXPHASE2NOREST_subject%d_%s_%s_%s.mat', outfold, loc2_shiftTRs, loc2_numTRs, subject, class_args.train_funct_name, class_args.test_funct_name, processed_params) ;
